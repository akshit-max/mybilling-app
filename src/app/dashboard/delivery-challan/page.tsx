...
export default function CreditNotePage() {
  ...
  const [deliveryChallans, setDeliveryChallans] = useState<CreditNote[]>([]);
  const [loading, setLoading] = useState(true);
  const [search, setSearch] = useState("");
  const [dateFilter, setDateFilter] = useState("365");

  const fetchDeliveryChallans = async (userId: string) => {
    try {
      setLoading(true);
      const q = query(collection(db, "deliveryChallans"), where("userId", "==", userId));
      const snap = await getDocs(q);
      const data: CreditNote[] = snap.docs.map((d) => {
        const doc = d.data();
        return {
          id: d.id,
          deliveryChallanNumber: doc.deliveryChallanNumber || "",
  ...
  useEffect(() => {
    const unsub = onAuthStateChanged(auth, (user) => {
      if (user) fetchDeliveryChallans(user.uid);
  ...
  const handleDelete = async (id: string, noteNumber: string) => {
    if (!confirm(`Delete Delivery Challan ${noteNumber}? This cannot be undone.`)) return;
    try {
  ...
  const now = new Date();
  const filtered = deliveryChallans.filter((c) => {
    if (search && !c.customerName.toLowerCase().includes(search.toLowerCase()) && !c.deliveryChallanNumber.toLowerCase().includes(search.toLowerCase())) return false;
  ...
  return (
    <div className="max-w-7xl mx-auto w-full flex flex-col min-h-[calc(100vh-80px)]">
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        <Link
          href="/dashboard/delivery-challan/create"
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-gray-50 border-b border-gray-100 text-[11px] uppercase tracking-wider text-gray-500">
                ...
                <th className="p-4 font-bold">Delivery Challan Number</th>
              ...
              {loading ? (
                ...
                <tr>
                  <td colSpan={7} className="p-16 text-center">
                    <div className="flex flex-col items-center justify-center text-gray-400">
                      ...
                      <Link href="/dashboard/delivery-challan/create" className="mt-4 text-sm font-bold text-indigo-600 hover:underline">
                        + Create Delivery Challan
                      </Link>
                    </div>
                  </td>
                ...
                filtered.map((cn) => (
                  <tr
                    ...
                    onClick={() => router.push(`/dashboard/delivery-challan/${cn.id}`)}
                  >
                    ...
                    <td className="p-4 font-bold text-gray-800 uppercase tracking-wide">{cn.deliveryChallanNumber}</td>
                    ...
                    <td className="p-4 text-center" onClick={(e) => e.stopPropagation()}>
                      <div className="flex items-center justify-center gap-2 opacity-100 transition">
                        ...
                        <button
                          onClick={() => router.push(`/dashboard/delivery-challan/edit/${cn.id}`)}
                        ...
                        <button
                          onClick={() => handleDelete(cn.id, cn.deliveryChallanNumber)}
