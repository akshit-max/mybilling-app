...
export default function ViewCreditNote() {
  ...
  const [deliveryChallan, setCreditNote] = useState<CreditNote | null>(null);
  ...
  useEffect(() => {
    const fetchCreditNote = async () => {
      try {
        const ref = doc(db, "deliveryChallans", id);
  ...
  if (loading) {
    return (
      <div className="min-h-[70vh] flex items-center justify-center text-gray-400 gap-2 font-sans">
        ...
        <span className="text-xs font-semibold uppercase tracking-wider">Loading Delivery Challan details...</span>
      </div>
    );
  }
  ...
  if (!deliveryChallan) {
    return (
      <div className="min-h-[70vh] flex flex-col items-center justify-center text-gray-400 gap-3 font-sans">
        ...
        <span className="text-sm font-semibold uppercase tracking-wider">Delivery Challan details not found</span>
      </div>
    );
  }

  const formattedDate = deliveryChallan.date
    ? new Date(deliveryChallan.date).toLocaleDateString()
    : deliveryChallan.createdAt
    ? typeof (deliveryChallan.createdAt as any).toDate === "function"
      ? (deliveryChallan.createdAt as any).toDate().toLocaleDateString()
      : new Date(deliveryChallan.createdAt as any).toLocaleDateString()
    : new Date().toLocaleDateString();

  const totalQty = deliveryChallan.items
    ? deliveryChallan.items.reduce((acc, item) => acc + (Number(item.qty) || 0), 0)
    : 0;

  const totalTaxAmount = deliveryChallan.items
    ? deliveryChallan.items.reduce((acc, item) => {
  ...
  const receivedAmount = typeof deliveryChallan.amountReceived === "number" ? deliveryChallan.amountReceived : 0;
  const balanceAmount = Math.max(0, deliveryChallan.total - receivedAmount);
  ...
  return (
    <div className="flex flex-col flex-1 min-w-0 font-sans bg-gray-50/60 min-h-screen">
      ...
      <div className="screen-only-container flex flex-col flex-1">
        ...
        <div className="bg-white border-b border-gray-200 px-6 py-3 flex items-center justify-between shrink-0 shadow-xs">
          <div className="flex items-center gap-2">
            <Link href="/dashboard/delivery-challan" className="text-gray-400 hover:text-gray-700 transition">
              <ArrowLeft size={16} />
            </Link>
            ...
            <div className="relative">
              ...
              {isMoreOpen && (
                <div className="absolute right-0 mt-1 bg-white border border-gray-200 rounded-lg shadow-md py-1 w-32 z-50 text-xs font-semibold">
                  <Link href={`/dashboard/delivery-challan/edit/${id}`} className="block px-4 py-2 text-gray-700 hover:bg-gray-50">Edit Delivery Challan</Link>
          ...
          >
             <div>
                ...
                <div className="grid grid-cols-2 border-y border-gray-300 bg-gray-50/60 px-4 py-2 mb-4 text-[10px] font-bold text-gray-700">
                   <div>
                     <p>Delivery Challan No.: <span className="font-mono text-gray-950 font-extrabold">{deliveryChallan.deliveryChallanNumber}</span></p>
                     {deliveryChallan.linkedInvoiceNumber && <p>Linked Invoice No.: <span className="font-mono text-gray-950 font-extrabold">{deliveryChallan.linkedInvoiceNumber}</span></p>}
                   </div>
                ...
                <div className="mb-4 space-y-0.5 text-[10px]">
                   ...
                   <p className="text-xs font-extrabold text-gray-900">{deliveryChallan.customerName}</p>
                   {showPhone && deliveryChallan.customerPhone && <p className="text-gray-650 font-semibold">Mobile: {deliveryChallan.customerPhone}</p>}
                   {deliveryChallan.customerGSTIN && <p className="text-gray-650 font-mono">GSTIN: {deliveryChallan.customerGSTIN}</p>}
                </div>
      ...
      {showWhatsAppModal && (
        <WhatsAppModal
          customerName={deliveryChallan?.customerName || "Customer"}
          existingPhone={deliveryChallan?.customerPhone}
          message={`Dear ${deliveryChallan?.customerName},\n\nYour Delivery Challan has been generated.\n\nTotal Amount: *₹${deliveryChallan?.total?.toFixed(2)}*\n\nThank you for choosing ${company?.name || "our company"}.`}
      ...
      {showSMSModal && (
        <SMSModal
          customerName={deliveryChallan?.customerName || "Customer"}
          invoiceNumber={(deliveryChallan as any)?.invoiceNumber || (deliveryChallan as any)?.quotationNumber || (deliveryChallan as any)?.challanNumber || (deliveryChallan as any)?.proformaInvoiceNumber || (deliveryChallan as any)?.orderNumber || (deliveryChallan as any)?.returnNumber || (deliveryChallan as any)?.debitNoteNumber || (deliveryChallan as any)?.creditNoteNumber || (deliveryChallan as any)?.receiptNumber || (deliveryChallan as any)?.id || "N/A"}
          moduleName="Delivery Challan"
          existingPhone={deliveryChallan?.customerPhone}
          message={`Dear ${deliveryChallan?.customerName},\n\nYour ${(deliveryChallan as any)?.invoiceType === 'estimate' ? 'Estimate' : 'Invoice'} has been generated.\n\nTotal Amount: *₹${deliveryChallan?.total?.toFixed(2)}*\n\nThank you for choosing ${company?.name || "our company"}.`}
      ...
      {showEmailModal && (
        <EmailModal
          ...
          documentNumber={deliveryChallan?.deliveryChallanNumber || "1"}
          totalAmount={deliveryChallan?.total || 0}
