...
export default function EditAutomatedBill() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        // Load Automated Bill
        try {
          const snap = await getDoc(doc(db, "automatedBills", id));
          if (snap.exists()) {
            ...
            if (billData.startDate) setStartDate(billData.startDate);
            if (billData.endDate) setEndDate(billData.endDate);
            if (billData.repeatFrequency) setRepeatFrequency(billData.repeatFrequency.toString());
            if (billData.repeatUnit) setRepeatUnit(billData.repeatUnit);
            if (billData.paymentTerms) setPaymentTerms(billData.paymentTerms.toString());
            if (billData.status) setStatus(billData.status);

            if (billData.shippingAddress) setShippingAddress(billData.shippingAddress);
            if (billData.notes) {
              setNotes(billData.notes);
              setShowNotes(true);
            }
            if (billData.discountType) setDiscountType(billData.discountType);
            if (billData.discountValue) {
              setDiscountValue(billData.discountValue);
              setShowDiscountInput(true);
            }
            if (billData.additionalChargeName) setAdditionalChargeName(billData.additionalChargeName);
            if (billData.additionalChargeValue) setAdditionalChargeValue(billData.additionalChargeValue);
          } else {
          ...
          console.error("Failed to load automated bill", e);
        }
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const invoiceData = {
        ...
        status: status,
        updatedAt: new Date(),
        nextInvoiceDate: startDate, // naive compute, backend will do properly
        shippingAddress,
        notes,
      ...
      await updateDoc(doc(db, "automatedBills", id), invoiceData);

      toast.success("Automated Bill updated successfully! ✅");
      router.push(`/dashboard/automated-bills/${id}`);
      ...
      <header className="bg-white border-b border-gray-200 px-6 py-3 flex items-center justify-between shadow-xs">
        <div className="flex items-center gap-4">
          ...
          <div className="space-y-0.5">
            <h1 className="text-sm font-bold text-gray-800 uppercase tracking-wider">Edit Automated Bill</h1>
            <span className="text-[10px] text-indigo-600 font-bold uppercase tracking-wider">Modify Schedule</span>
          </div>
        </div>
      ...
      <main className="flex-1 max-w-[1400px] w-full mx-auto p-6 space-y-6">
        ...
        <div className="bg-white border border-gray-200 rounded-lg shadow-sm overflow-hidden">
          ...
          <div className="p-6 grid grid-cols-1 lg:grid-cols-3 gap-6 border-b border-gray-100 bg-gray-50/20">
            ...
            {/* Extra Row for Status in Edit View */}
            <div className="bg-white border border-gray-200 rounded-lg p-4 mt-4 shadow-xs">
              <label className="block text-[9px] font-bold text-gray-400 uppercase tracking-wider mb-1">Billing Status</label>
              <select
                value={status}
                onChange={(e) => setStatus(e.target.value as "Active" | "Paused" | "Stopped")}
                className="w-full border-b border-gray-200 py-1 text-xs focus:outline-none focus:border-indigo-500 font-bold bg-white cursor-pointer"
                style={{
