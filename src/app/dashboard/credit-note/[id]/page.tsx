...
export default function ViewCreditNote() {
  ...
  useEffect(() => {
    const fetchCreditNote = async () => {
      try {
        const ref = doc(db, "creditNotes", id);
  ...
  if (loading) {
    return (
      <div className="min-h-[70vh] flex items-center justify-center text-gray-400 gap-2 font-sans">
        ...
        <span className="text-xs font-semibold uppercase tracking-wider">Loading Credit Note details...</span>
      </div>
    );
  }
  ...
  if (!creditNote) {
    return (
      <div className="min-h-[70vh] flex flex-col items-center justify-center text-gray-400 gap-3 font-sans">
        ...
        <span className="text-sm font-semibold uppercase tracking-wider">Credit Note details not found</span>
      </div>
    );
  }

  const formattedDate = creditNote.date
    ? new Date(creditNote.date).toLocaleDateString()
    : creditNote.createdAt
    ? typeof (creditNote.createdAt as any).toDate === "function"
      ? (creditNote.createdAt as any).toDate().toLocaleDateString()
      : new Date(creditNote.createdAt as any).toLocaleDateString()
    : new Date().toLocaleDateString();

  const totalQty = creditNote.items
    ? creditNote.items.reduce((acc, item) => acc + (Number(item.qty) || 0), 0)
    : 0;

  const totalTaxAmount = creditNote.items
    ? creditNote.items.reduce((acc, item) => {
        const taxRate = item.tax || (creditNote.gstEnabled ? 18 : 0);
        const itemAmount = (Number(item.qty) || 0) * (Number(item.price) || 0);
        return acc + (itemAmount * (taxRate / 100));
      }, 0)
    : 0;

  const receivedAmount = typeof creditNote.amountReceived === "number" ? creditNote.amountReceived : 0;
  const balanceAmount = Math.max(0, creditNote.total - receivedAmount);
  ...
  return (
    <div className="flex flex-col flex-1 min-w-0 font-sans bg-gray-50/60 min-h-screen">
      ...
      <div className="screen-only-container flex flex-col flex-1">
        ...
        <div className="bg-white border-b border-gray-200 px-6 py-3 flex items-center justify-between shrink-0 shadow-xs">
          <div className="flex items-center gap-2">
            <Link href="/dashboard/credit-note" className="text-gray-400 hover:text-gray-700 transition">
              <ArrowLeft size={16} />
            </Link>
            <h1 className="text-base font-bold text-gray-800 flex items-center gap-2">
              <span>Credit Note #{creditNote.creditNoteNumber}</span>
              <span className={`text-[10px] font-bold uppercase tracking-wider px-2 py-0.5 rounded-full ${
                creditNote.status === "adjusted" 
            ...
            <div className="relative">
              ...
              {isMoreOpen && (
                <div className="absolute right-0 mt-1 bg-white border border-gray-200 rounded-lg shadow-md py-1 w-32 z-50 text-xs font-semibold">
                  <Link href={`/dashboard/credit-note/edit/${id}`} className="block px-4 py-2 text-gray-700 hover:bg-gray-50">Edit Credit Note</Link>
          ...
          >
             <div>
                ...
                <div className="grid grid-cols-2 border-y border-gray-300 bg-gray-50/60 px-4 py-2 mb-4 text-[10px] font-bold text-gray-700">
                   <div>
                     <p>Credit Note No.: <span className="font-mono text-gray-950 font-extrabold">{creditNote.creditNoteNumber}</span></p>
                     {creditNote.linkedInvoiceNumber && <p>Linked Invoice No.: <span className="font-mono text-gray-950 font-extrabold">{creditNote.linkedInvoiceNumber}</span></p>}
                   </div>
                ...
                <div className="mb-4 space-y-0.5 text-[10px]">
                   ...
                   <p className="text-xs font-extrabold text-gray-900">{creditNote.customerName}</p>
                   {showPhone && creditNote.customerPhone && <p className="text-gray-650 font-semibold">Mobile: {creditNote.customerPhone}</p>}
                   {creditNote.customerGSTIN && <p className="text-gray-650 font-mono">GSTIN: {creditNote.customerGSTIN}</p>}
                </div>
                ...
                <div className="border border-gray-300 rounded overflow-hidden mb-4">
                   <table className="w-full text-[10px] text-left border-collapse">
                      ...
                      <tbody className="divide-y divide-gray-250 text-gray-700 font-semibold">
                         {creditNote.items && creditNote.items.map((item, idx) => {
                           const taxRate = item.tax || (creditNote.gstEnabled ? 18 : 0);
                           return (
                         ...
                         <tr className="bg-gray-50/50 font-bold border-y-2 border-gray-300 text-gray-900 text-[10px]">
                            ...
                            <td className="py-2 px-3 text-right font-mono">₹{creditNote.subtotal.toFixed(2)}</td>
                         </tr>
                      </tbody>
                   </table>
                </div>
                ...
                <div className="flex justify-between items-start text-[10px] pt-2">
                   ...
                   <div className="w-64 space-y-1 font-mono text-right text-gray-500 font-bold border-t border-dashed border-gray-350 pt-2">
                      <div className="flex justify-between text-gray-600">
                         ...
                         <span className="text-gray-900">₹{creditNote.subtotal.toFixed(2)}</span>
                      </div>

                      {creditNote.gstEnabled && (
                        creditNote.isInterstate ? (
                          <div className="flex justify-between text-gray-500">
                            <span>IGST</span>
                            <span>₹{(creditNote.igst || 0).toFixed(2)}</span>
                          </div>
                        ) : (
                          <>
                            <div className="flex justify-between text-gray-500">
                              <span>CGST</span>
                              <span>₹{creditNote.cgst.toFixed(2)}</span>
                            </div>
                            <div className="flex justify-between text-gray-500">
                              <span>SGST</span>
                              <span>₹{creditNote.sgst.toFixed(2)}</span>
                            </div>
                          </>
                        )
                      )}
                      ...
                      {creditNote.discountAmount > 0 && (
                        <div className="flex justify-between text-brand-tertiary">
                           <span>Discount</span>
                           <span>-₹{creditNote.discountAmount.toFixed(2)}</span>
                        </div>
                      )}
                      ...
                      <div className="flex justify-between text-xs font-black text-gray-900 border-y border-gray-350 py-1 mt-1 bg-gray-50 px-1">
                         ...
                         <span className="font-extrabold text-black">₹{creditNote.total.toFixed(2)}</span>
                      </div>
             ...
             <div className="mt-8 pt-2 flex flex-col justify-between">
                <div className="flex justify-between items-end text-[9px] text-gray-400">
                  <div>
                    ...
                    <p className="italic font-bold text-gray-800 mt-0.5">{numberToWords(creditNote.total)} Rupees Only</p>
                  </div>
                  {(creditNote.signatureType === "empty" || creditNote.signatureType === "upload") && (
                    <div className="text-right space-y-1 w-44">
                      ...
                      {creditNote.signatureType === "empty" ? (
                        ...
                        <div className="h-12 border border-gray-200 rounded flex items-center justify-center p-1 bg-white overflow-hidden">
                          <img src={creditNote.signatureImage} alt="Signature" className="max-h-full max-w-full object-contain" />
                        </div>
                      )}
                    </div>
                  )}
      ...
      {showSMSModal && (
        <SMSModal
          customerName={creditNote?.customerName || "Customer"}
          invoiceNumber={(creditNote as any)?.invoiceNumber || (creditNote as any)?.quotationNumber || (creditNote as any)?.challanNumber || (creditNote as any)?.proformaInvoiceNumber || (creditNote as any)?.orderNumber || (creditNote as any)?.returnNumber || (creditNote as any)?.debitNoteNumber || (creditNote as any)?.creditNoteNumber || (creditNote as any)?.receiptNumber || (creditNote as any)?.id || "N/A"}
      ...
      {showEmailModal && (
        <EmailModal
          ...
          documentNumber={creditNote?.creditNoteNumber || "1"}
          totalAmount={creditNote?.total || 0}
