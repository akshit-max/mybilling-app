...
export default function CreditNotePage() {
  ...
  const fetchCreditNotes = async (userId: string) => {
    try {
      setLoading(true);
      const q = query(collection(db, "creditNotes"), where("userId", "==", userId));
  ...
  const filtered = creditNotes.filter((c) => {
    if (search && !c.customerName.toLowerCase().includes(search.toLowerCase()) && !c.creditNoteNumber.toLowerCase().includes(search.toLowerCase())) return false;
  ...
  return (
    <div className="max-w-7xl mx-auto w-full flex flex-col min-h-[calc(100vh-80px)]">
      ...
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-xl font-bold text-gray-900 tracking-tight">Credit Note</h1>
        <Link
          href="/dashboard/credit-note/create"
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-gray-50 border-b border-gray-100 text-[11px] uppercase tracking-wider text-gray-500">
                ...
                <th className="p-4 font-bold">Credit Note Number</th>
              ...
              {loading ? (
                ...
                <tr>
                  <td colSpan={7} className="p-16 text-center">
                    <div className="flex flex-col items-center justify-center text-gray-400">
                      ...
                      <Link href="/dashboard/credit-note/create" className="mt-4 text-sm font-bold text-indigo-600 hover:underline">
                        + Create Credit Note
                      </Link>
                    </div>
                  </td>
                ...
                filtered.map((cn) => (
                  <tr
                    ...
                    onClick={() => router.push(`/dashboard/credit-note/${cn.id}`)}
                  >
                    ...
                    <td className="p-4 font-bold text-gray-800 uppercase tracking-wide">{cn.creditNoteNumber}</td>
                    ...
                    <td className="p-4 text-center" onClick={(e) => e.stopPropagation()}>
                      <div className="flex items-center justify-center gap-2 opacity-100 transition">
                        ...
                        <button
                          onClick={() => router.push(`/dashboard/credit-note/edit/${cn.id}`)}
