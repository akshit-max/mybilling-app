...
export default function QuotationsPage() {
  ...
  const getStatusStyle = (status: string) => {
    ...
    if (s === "credit") return "bg-amber-50 text-amber-700 border border-amber-100";
  ...
  const filteredInvoices = invoices.filter((inv) => {
    ...
    // 2. Status Filter
    const statusMatch = statusFilter === "all" || (inv.status === "pending" ? "Open" : inv.status).toLowerCase() === statusFilter;
            ...
            <table className="w-full text-left text-xs text-gray-600 border-collapse">
              <thead>
                <tr className="bg-gray-50 border-b border-gray-200 text-gray-400 uppercase tracking-wider text-[10px] font-semibold">
                  ...
                  <th className="px-4 py-3 font-semibold">Quotation Number</th>
              ...
              <tbody className="divide-y divide-gray-150">
                {filteredInvoices.map((inv) => {
                  const isOverdue = inv.dueDate && new Date(inv.dueDate) < new Date() && (inv.status === "pending" ? "Open" : inv.status) !== "paid" && (inv.status === "pending" ? "Open" : inv.status) !== "cancelled";
                  ...
                  return (
                    <tr key={inv.id} className="hover:bg-gray-50/45 transition-colors">
                      ...
                      <td className="px-4 py-3">
                        <span className={`px-2 py-0.5 text-[10px] rounded-full font-bold uppercase ${getStatusStyle((inv.status === "pending" ? "Open" : inv.status))}`}>
                      ...
                      <td className="px-4 py-3 text-center relative">
                        ...
                        {activeDropdownId === inv.id && (
                          ...
                          >
                            ...
                            {(inv.status === "pending" ? "Open" : inv.status) !== "cancelled" && (
                              <button 
