...
export default function CreditNotePage() {
  ...
  const [debitNotes, setCreditNotes] = useState<CreditNote[]>([]);
  const [loading, setLoading] = useState(true);
  const [search, setSearch] = useState("");
  const [dateFilter, setDateFilter] = useState("365");

  const fetchCreditNotes = async (userId: string) => {
    try {
      setLoading(true);
      const q = query(collection(db, "debitNotes"), where("userId", "==", userId));
      const snap = await getDocs(q);
      const data: CreditNote[] = snap.docs.map((d) => {
        const doc = d.data();
        return {
          id: d.id,
          debitNoteNumber: doc.debitNoteNumber || "",
  ...
  useEffect(() => {
    const unsub = onAuthStateChanged(auth, (user) => {
      if (user) fetchCreditNotes(user.uid);
  ...
  const handleDelete = async (id: string, noteNumber: string) => {
    if (!confirm(`Delete Debit Note ${noteNumber}? This cannot be undone.`)) return;
    try {
  ...
  const now = new Date();
  const filtered = debitNotes.filter((c) => {
    if (search && !c.customerName.toLowerCase().includes(search.toLowerCase()) && !c.debitNoteNumber.toLowerCase().includes(search.toLowerCase())) return false;
  ...
  return (
    <div className="max-w-7xl mx-auto w-full flex flex-col min-h-[calc(100vh-80px)]">
      ...
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-xl font-bold text-gray-900 tracking-tight">Debit Note</h1>
        <Link
          href="/dashboard/debit-note/create"
      ...
      <div className="bg-white border border-gray-200 rounded-xl shadow-xs overflow-hidden flex-1 flex flex-col">
        ...
        <div className="p-4 flex flex-col sm:flex-row gap-3 items-center border-b border-gray-100 bg-gray-50/40">
          <div className="relative w-full sm:w-64">
            ...
            <input
              type="text"
              placeholder="Search party or debit note no..."
              value={search}
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-gray-50 border-b border-gray-100 text-[11px] uppercase tracking-wider text-gray-500">
                ...
                <th className="p-4 font-bold">Debit Note Number</th>
              ...
              {loading ? (
                ...
                <tr>
                  <td colSpan={7} className="p-16 text-center">
                    <div className="flex flex-col items-center justify-center text-gray-400">
                      ...
                      <Link href="/dashboard/debit-note/create" className="mt-4 text-sm font-bold text-indigo-600 hover:underline">
                        + Create Debit Note
                      </Link>
                    </div>
                  </td>
                ...
                filtered.map((cn) => (
                  <tr
                    ...
                    onClick={() => router.push(`/dashboard/debit-note/${cn.id}`)}
                  >
                    ...
                    <td className="p-4 font-bold text-gray-800 uppercase tracking-wide">{cn.debitNoteNumber}</td>
                    ...
                    <td className="p-4 text-center" onClick={(e) => e.stopPropagation()}>
                      <div className="flex items-center justify-center gap-2 opacity-0 group-hover:opacity-100 transition">
                        ...
                        <button
                          onClick={() => router.push(`/dashboard/debit-note/edit/${cn.id}`)}
                        ...
                        <button
                          onClick={() => handleDelete(cn.id, cn.debitNoteNumber)}
