...
export default function ItemSalesSummaryReport() {
  ...
  useEffect(() => {
    const unsub = onAuthStateChanged(auth, async (user) => {
      ...
      try {
        ...
        const salesMap = new Map<string, ItemSales>();
        ...
        invSnap.docs.forEach(doc => {
          ...
          if (Array.isArray(inv.items)) {
            inv.items.forEach((item: any) => {
              ...
              const total = Number(item.amount || item.total || ((item.qty || 0) * (item.price || item.rate || 0)) || 0);

              if (!salesMap.has(itemName)) {
                salesMap.set(itemName, {
                  id: itemName, // Using name as ID since items in invoices might not have DB IDs
                  name: itemName,
                  quantitySold: 0,
                  totalAmount: 0
                });
              ...
              const existing = salesMap.get(itemName)!;
        ...
        data.sort((a, b) => b.totalAmount - a.totalAmount); // Sort by highest sales amount
        setItems(data);
      } catch (err) {
        console.error("Fetch error:", err);
        toast.error("Failed to load item sales summary.");
      } finally {
        setLoading(false);
      }
    });
  ...
  const totalSalesQuantity = filteredItems.reduce((acc, curr) => acc + curr.quantitySold, 0);
  const totalSalesAmount = filteredItems.reduce((acc, curr) => acc + curr.totalAmount, 0);
  ...
  const handleExportExcel = () => {
    ...
    const headers = ["Item Name", "Quantity Sold", "Total Amount"];
    ...
    // Add total row
    rows.push(["Total", totalSalesQuantity.toString(), totalSalesAmount.toString()]);
    ...
    const tableHTML = `
      ...
      <body>
        <h2>Item Sales Summary Report</h2>
        <table>
          <thead>
    ...
    link.href = url;
    link.download = `Item_Sales_Summary.xls`;
  ...
  const handleEmailExcel = async () => {
    ...
    const headers = ["Item Name", "Quantity Sold", "Total Sales Amount (₹)"];
    const rows = filteredItems.map(p => [
      p.name,
      p.quantitySold.toString(),
      p.totalAmount.toLocaleString('en-IN')
    ]);
    rows.push(["Total", totalSalesQuantity.toString(), totalSalesAmount.toLocaleString('en-IN')]);

    const tableHTML = `
      <div style="font-family: Arial, sans-serif; padding: 20px;">
        <h2 style="color: #1e3a8a;">Item Sales Summary Report</h2>
        ...
        <table style="border-collapse: collapse; width: 100%; margin-top: 16px;">
          ...
          <tbody>
            ${rows.map((row, i) => `<tr style="${i === rows.length - 1 ? "font-weight:bold;background:#f9fafb;" : ""}">${row.map(cell => `<td style="border: 1px solid #e5e7eb; padding: 6px; color: #374151;">${cell}</td>`).join("")}</tr>`).join("")}
          </tbody>
        </table>
      </div>
    `;
    ...
    await toast.promise(
      fetch("/api/send-email", {
        ...
        body: JSON.stringify({
          to: emails,
          subject: "Item Sales Summary Report",
          html: "<p>Please find the attached " + "Item Sales Summary Report" + " Excel report.</p>",
          attachments: [
            {
              filename: "Item Sales Summary Report".replace(/\s+/g, '_') + ".xls",
    ...
    <div id="print-area" className="space-y-0 max-w-full mx-auto pb-10 font-sans bg-gray-50/50 min-h-screen print:bg-white print:pb-0">
      ...
      <div className="flex justify-between items-center bg-white px-6 py-3 border-b border-gray-200 shadow-sm print:hidden">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-base font-bold text-gray-800">Item Sales Summary</h1>
          <button 
            onClick={() => {
      ...
      <div className="hidden print:block text-center py-6 border-b border-gray-200 mb-4">
        <h1 className="text-2xl font-bold text-gray-800 uppercase tracking-wider">Item Sales Summary</h1>
      ...
      <div className="px-6 pt-4 pb-2 print:hidden flex justify-between items-center">
        ...
        <div className="flex gap-4">
          <div className="bg-white border border-gray-200 px-4 py-1.5 rounded shadow-sm text-xs font-bold text-gray-700">
            Total Qty: <span className="text-indigo-600 ml-1">{totalSalesQuantity.toLocaleString('en-IN')}</span>
          </div>
          <div className="bg-white border border-gray-200 px-4 py-1.5 rounded shadow-sm text-xs font-bold text-gray-700">
            Total Sales: <span className="text-indigo-600 ml-1">₹ {totalSalesAmount.toLocaleString('en-IN')}</span>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white border-y sm:border sm:rounded-lg border-gray-200 mx-0 sm:mx-6 mt-2 mb-10 overflow-visible print:border-none print:shadow-none print:mx-0 print:mt-0 shadow-sm print:overflow-visible">
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-20 text-gray-500 print:hidden">
            ...
            <p className="text-xs mt-1">No items have been sold yet or match your search.</p>
          </div>
        ) : (
            ...
            <table className="w-full text-left text-xs border-collapse">
              <thead>
                <tr className="bg-gray-50/80 border-b border-gray-200 text-gray-500 font-semibold uppercase tracking-wider text-[10px] print:bg-gray-100">
                  ...
                  <th className="px-4 py-3 border-r border-gray-100 print:border-gray-300 text-center">Quantity Sold</th>
                  <th className="px-4 py-3 text-right print:border-gray-300">Total Sales Amount</th>
                </tr>
              </thead>
                ...
                {filteredItems.map((p) => (
                  <tr key={p.id} className="hover:bg-indigo-50/30 transition-colors group">
                    ...
                    <td className="px-4 py-3 font-mono font-bold text-gray-800 text-right print:border-gray-300">
                      ₹ {p.totalAmount.toLocaleString('en-IN', { minimumFractionDigits: 0 })}
                    </td>
                  </tr>
                ))}
                {/* Total Row */}
                <tr className="bg-gray-50 print:bg-gray-100 border-t border-gray-200">
                  <td className="px-4 py-3 text-right font-bold text-gray-800 print:border-gray-300 border-r border-gray-100">
                    Total
                  </td>
                  <td className="px-4 py-3 font-mono font-bold text-indigo-700 text-center print:border-gray-300 print:text-gray-800 border-r border-gray-100">
                    {totalSalesQuantity.toLocaleString('en-IN')}
                  </td>
                  <td className="px-4 py-3 font-mono font-bold text-indigo-700 text-right print:border-gray-300 print:text-gray-800">
                    ₹ {totalSalesAmount.toLocaleString('en-IN', { minimumFractionDigits: 0 })}
                  </td>
                </tr>
              </tbody>
            </table>
