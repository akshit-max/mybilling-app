...
import { ArrowLeft, Star, Mail, Download, Printer, Search, X, ChevronDown, AlertTriangle } from "lucide-react";
...
export default function LowStockSummaryReport() {
    ...
    const unsub = onAuthStateChanged(auth, async (user) => {
      ...
      try {
        ...
        snap.docs.forEach(doc => {
          ...
          const threshold = Number(p.lowStockThreshold || 2);
        ...
        toast.error("Failed to load low stock summary.");
      } finally {
        setLoading(false);
      }
    });
  ...
  const handleExportExcel = () => {
    ...
    const headers = ["Item Name", "Item Code", "Category", "Current Stock", "Low Stock Threshold"];
    ...
    const tableHTML = `
      ...
      <body>
        <h2>Low Stock Summary Report</h2>
        <table>
          <thead>
    ...
    link.href = url;
    link.download = `Low_Stock_Summary.xls`;
  ...
  const handleEmailExcel = async () => {
    ...
    const tableHTML = `
      <div style="font-family: Arial, sans-serif; padding: 20px;">
        <h2 style="color: #1e3a8a;">Low Stock Summary Report</h2>
    ...
    <div id="print-area" className="space-y-0 max-w-full mx-auto pb-10 font-sans bg-gray-50/50 min-h-screen print:bg-white print:pb-0">
      ...
      <div className="flex justify-between items-center bg-white px-6 py-3 border-b border-gray-200 shadow-sm print:hidden">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-base font-bold text-gray-800">Low Stock Summary</h1>
          <button 
            onClick={() => {
      ...
      <div className="hidden print:block text-center py-6 border-b border-gray-200 mb-4">
        <h1 className="text-2xl font-bold text-gray-800 uppercase tracking-wider">Low Stock Summary</h1>
        <p className="text-sm text-gray-500 mt-1 font-semibold">Critical Items</p>
        ...
        {loading ? (
          ...
          <div className="overflow-x-auto print:overflow-visible">
            <table className="w-full text-left text-xs border-collapse">
              <thead>
                <tr className="bg-amber-50/80 border-b border-amber-200 text-amber-700 font-semibold uppercase tracking-wider text-[10px] print:bg-gray-100 print:text-gray-600">
                  <th className="px-4 py-3 border-r border-amber-100 print:border-gray-300">Item Name</th>
                  <th className="px-4 py-3 border-r border-amber-100 print:border-gray-300">Item Code</th>
                  <th className="px-4 py-3 border-r border-amber-100 print:border-gray-300">Category</th>
                  <th className="px-4 py-3 border-r border-amber-100 print:border-gray-300 text-center">Current Stock</th>
                  <th className="px-4 py-3 text-center print:border-gray-300">Minimum Threshold</th>
                </tr>
              </thead>
                ...
                {filteredItems.map((p) => {
                  ...
                  return (
                    <tr key={p.id} className="hover:bg-amber-50/20 transition-colors group">
                      ...
                      <td className="px-4 py-3 font-mono font-bold text-center border-r border-gray-50 print:border-gray-300">
                        <span className={isOutOfStock ? "text-red-600" : "text-amber-600"}>
                          {p.stock}
                        </span>
                      </td>
                      <td className="px-4 py-3 font-mono font-semibold text-gray-500 text-center print:border-gray-300">
                        {p.threshold}
                      </td>
                    </tr>
                  );
