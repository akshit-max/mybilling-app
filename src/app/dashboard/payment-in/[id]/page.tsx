...
export default function PaymentInReceipt() {
  ...
  useEffect(() => {
    const fetchPayment = async () => {
      try {
        const ref = doc(db, "paymentIn", id);
        const snap = await getDoc(ref);
        
        if (snap.exists()) {
          setPayment({ id: snap.id, ...snap.data() } as PaymentIn);
        } else {
    ...
    <div className="flex flex-col flex-1 min-w-0 font-sans min-h-screen pb-20 bg-gray-50/60">
      ...
      <div className="flex items-center justify-between mb-6 print:hidden">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-xl font-bold text-gray-900 tracking-tight">Payment In #{payment.paymentNumber}</h1>
        </div>
        <div className="flex items-center gap-3">
          <Link href={`/dashboard/payment-in/edit/${payment.id}`} className="flex items-center gap-2 bg-white border border-gray-200 text-gray-600 px-4 py-2 rounded-lg text-sm font-semibold hover:bg-gray-50 transition shadow-sm">
            <Edit size={16} />
            Edit
          </Link>
        ...
        <div className="p-6">
           ...
           <div className="grid grid-cols-2 md:grid-cols-4 gap-6 mb-8">
              ...
              <div>
                 ...
                 <p className="font-bold text-indigo-700 text-xl">₹{payment.amountReceived.toLocaleString("en-IN", { minimumFractionDigits: 2 })}</p>
              </div>
              <div>
           ...
           {/* SETTLED INVOICES */}
           <h3 className="font-bold text-gray-800 mb-4 text-sm mt-8">Invoices settled with this payment</h3>
      ...
      <div className="print-only-container font-sans bg-white text-black p-8">
        <h1 className="text-2xl font-bold uppercase mb-6 text-center border-b pb-4">Payment Receipt</h1>
        <div className="flex justify-between items-start mb-8 text-sm">
          <div>
            <p className="font-bold text-gray-500 uppercase">Received From</p>
        ...
        <div className="mb-8 border border-gray-300 rounded p-4 flex justify-between items-center bg-gray-50">
          ...
          <span className="font-extrabold text-2xl text-black">₹{payment.amountReceived.toLocaleString("en-IN", { minimumFractionDigits: 2 })}</span>
        </div>
