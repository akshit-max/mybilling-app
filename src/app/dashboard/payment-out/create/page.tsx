...
export default function CreatePaymentOut() {
  ...
  const [paymentPrefix, setPaymentPrefix] = useState("NU/AB");
  ...
  // Invoices State
  const [unsettledInvoices, setUnsettledInvoices] = useState<PurchaseInvoice[]>([]);
  
  useEffect(() => {
    const init = onAuthStateChanged(auth, async (user) => {
      if (user) {
        // Fetch Parties (usually from 'customers' or a generic 'parties' collection)
        // Since many systems use 'customers' for both or have a 'parties' collection,
        // we query customers here assuming they act as general ledger parties.
        try {
          const cq = query(collection(db, "customers"), where("userId", "==", user.uid));
          const csnap = await getDocs(cq);
          setParties(csnap.docs.map((d) => ({ id: d.id, name: d.data().name })));
        } catch (e) {
        ...
        try {
          const pq = query(collection(db, "paymentOut"), where("userId", "==", user.uid));
  ...
  useEffect(() => {
    const fetchInvoicesForParty = async () => {
      ...
      try {
        // We query the 'purchases' collection!
        const q = query(
        ...
        const list: PurchaseInvoice[] = snap.docs
          .map((d) => {
            ...
            return {
              ...
              amountPaid: Number(data.amountPaid || 0), // purchases usually track amountPaid
              status: data.status || "pending",
              allocatedAmount: 0,
            };
          })
          .filter(inv => inv.status !== "paid" && (inv.total - inv.amountPaid) > 0);
    ...
    if (parties.find(c => c.name === partyName)) {
  ...
  useEffect(() => {
    const totalToAllocate = Number(amountPaid || 0) + Number(paymentDiscount || 0);
    if (unsettledInvoices.length > 0) {
      let remaining = totalToAllocate;
      const updated = unsettledInvoices.map(inv => {
        const maxAllowed = inv.total - inv.amountPaid;
  ...
  }, [amountPaid, paymentDiscount]);

  const updateAllocation = (index: number, val: string) => {
    const num = val === "" ? 0 : Number(val);
    const updated = [...unsettledInvoices];
    
    const maxAllowed = updated[index].total - updated[index].amountPaid;
    if (num > maxAllowed) {
  ...
  const handleSave = async () => {
    ...
    if (totalAvailable <= 0) return toast.error("Please enter a valid Amount Paid");
    ...
    try {
      ...
      for (const item of itemsToSettle) {
        ...
        if (inv) {
          const newAmountPaid = inv.amountPaid + item.amountSettled;
  ...
  const currentBalanceStr = "Current Balance: ₹" + unsettledInvoices.reduce((acc, inv) => acc + (inv.total - inv.amountPaid), 0).toFixed(2);
    ...
    <div className="max-w-7xl mx-auto w-full pb-20">
      ...
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          <Link href="/dashboard/payment-out" className="p-2 hover:bg-gray-200 rounded-full transition">
            <ArrowLeft size={20} className="text-gray-600" />
          </Link>
          <h1 className="text-xl font-bold text-gray-900 tracking-tight">Record Payment Out #{paymentNumber}</h1>
        </div>
        <div className="flex items-center gap-3">
          <button 
            onClick={() => router.back()}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
        ...
        <div className="bg-white border border-gray-200 rounded-xl shadow-xs p-5 space-y-4">
          <div className="relative">
            ...
            {showPartyDropdown && (
              <div onMouseDown={(e) => e.preventDefault()} className="absolute left-0 right-0 mt-1 bg-white border border-gray-200 rounded-lg shadow-lg max-h-48 overflow-y-auto z-20">
                ...
                {parties.length === 0 && <div className="px-3 py-2 text-xs text-gray-400">No parties found</div>}
              </div>
            )}
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-[10px] font-bold text-gray-400 uppercase tracking-wider mb-1.5">Amount Paid</label>
              <input
                type="number"
                placeholder="0"
                value={amountPaid}
            ...
            <div>
              <label className="block text-[10px] font-bold text-gray-400 uppercase tracking-wider mb-1.5">Payment Out Discount</label>
              <input
                type="number"
                placeholder="0"
        ...
        <div className="bg-white border border-gray-200 rounded-xl shadow-xs p-5 space-y-4">
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
             ...
             <div>
               <label className="block text-[10px] font-bold text-gray-400 uppercase tracking-wider mb-1.5">Payment Out Prefix</label>
               <input
                 type="text"
                 value={paymentPrefix}
             ...
             <div>
               <label className="block text-[10px] font-bold text-gray-400 uppercase tracking-wider mb-1.5">Payment Out Number</label>
               <input
                 type="text"
                 value={paymentNumber}
      ...
      <div className="bg-white border border-gray-200 rounded-xl shadow-xs">
         <div className="px-5 py-4 border-b border-gray-100 flex items-center justify-between">
           <h3 className="font-bold text-gray-800">Settle purchases with this payment</h3>
         </div>
         ...
         {!partyName ? (
            <div className="p-16 flex flex-col items-center justify-center text-center">
              ...
              <button 
                onClick={() => (document.querySelector('input[placeholder*="Search party"]') as HTMLElement)?.focus()}
            ...
            <div className="p-12 text-center text-gray-500 font-medium">
              No unpaid purchases found for {partyName}.
            </div>
         ) : (
                 ...
                 {unsettledInvoices.map((inv, idx) => (
                   <tr key={inv.id} className="border-b border-gray-50 hover:bg-gray-50/50 transition">
                     ...
                     <td className="p-4 text-sm font-bold text-gray-800">
                        ...
                        <span className="text-red-500 font-semibold text-xs ml-1">(₹{(inv.total - inv.amountPaid).toLocaleString()} pending)</span>
                     </td>
                     <td className="p-4">
                        <input
                          type="number"
