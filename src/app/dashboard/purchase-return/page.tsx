...
export default function CreditNotePage() {
  ...
  const [purchaseReturns, setCreditNotes] = useState<CreditNote[]>([]);
  ...
  const fetchCreditNotes = async (userId: string) => {
    try {
      setLoading(true);
      const q = query(collection(db, "purchaseReturns"), where("userId", "==", userId));
      const snap = await getDocs(q);
      const data: CreditNote[] = snap.docs.map((d) => {
        const doc = d.data();
        return {
          id: d.id,
          purchaseReturnNumber: doc.purchaseReturnNumber || "",
  ...
  const now = new Date();
  const filtered = purchaseReturns.filter((c) => {
    if (search && !c.customerName.toLowerCase().includes(search.toLowerCase()) && !c.purchaseReturnNumber.toLowerCase().includes(search.toLowerCase())) return false;
  ...
  return (
    <div className="max-w-7xl mx-auto w-full flex flex-col min-h-[calc(100vh-80px)]">
      ...
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-xl font-bold text-gray-900 tracking-tight">Purchase Return</h1>
        <Link
          href="/dashboard/purchase-return/create"
      ...
      <div className="bg-white border border-gray-200 rounded-xl shadow-xs overflow-hidden flex-1 flex flex-col">
        ...
        <div className="p-4 flex flex-col sm:flex-row gap-3 items-center border-b border-gray-100 bg-gray-50/40">
          <div className="relative w-full sm:w-64">
            ...
            <input
              type="text"
              placeholder="Search party or purchase return no..."
              value={search}
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-gray-50 border-b border-gray-100 text-[11px] uppercase tracking-wider text-gray-500">
                ...
                <th className="p-4 font-bold">Purchase Return Number</th>
              ...
              {loading ? (
                ...
                <tr>
                  <td colSpan={7} className="p-16 text-center">
                    <div className="flex flex-col items-center justify-center text-gray-400">
                      ...
                      <Link href="/dashboard/purchase-return/create" className="mt-4 text-sm font-bold text-indigo-600 hover:underline">
                        + Create Purchase Return
                      </Link>
                    </div>
                  </td>
                ...
                filtered.map((cn) => (
                  <tr
                    ...
                    onClick={() => router.push(`/dashboard/purchase-return/${cn.id}`)}
                  >
                    ...
                    <td className="p-4 font-bold text-gray-800 uppercase tracking-wide">{cn.purchaseReturnNumber}</td>
                    ...
                    <td className="p-4 text-center" onClick={(e) => e.stopPropagation()}>
                      <div className="flex items-center justify-center gap-2 opacity-100 transition">
                        ...
                        <button
                          onClick={() => router.push(`/dashboard/purchase-return/edit/${cn.id}`)}
