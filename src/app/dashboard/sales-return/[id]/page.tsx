...
export default function ViewInvoice() {
  ...
  useEffect(() => {
    const fetchInvoice = async () => {
      try {
        ...
        const ref = doc(db, "salesReturns", id);
  ...
  const salesReturnTypeTitle = (invoice.invoiceType || "invoice") === "estimate" ? "Estimate" : "Sales Return";
      ...
      <div className="screen-only-container flex flex-col flex-1">
        ...
        <div className="bg-white border-b border-gray-200 px-6 py-3 flex items-center justify-between shrink-0 shadow-xs">
          <div className="flex items-center gap-2">
            <Link href="/dashboard/sales-return" className="text-gray-400 hover:text-gray-700 transition">
              <ArrowLeft size={16} />
            </Link>
            ...
            <div className="relative">
              ...
              {isMoreOpen && (
                <div className="absolute right-0 mt-1 bg-white border border-gray-200 rounded-lg shadow-md py-1 w-32 z-50 text-xs font-semibold">
                  <Link href={`/dashboard/sales-return/edit/${id}`} className="block px-4 py-2 text-gray-700 hover:bg-gray-50">Edit Invoice</Link>
               ...
               <div>
                  ...
                  <div className="flex justify-between items-center mb-4">
                     <div className="flex items-center gap-2">
                        ...
                        >
                          {(invoice.invoiceType || "invoice") === "estimate" ? "ESTIMATE" : "SALES RETURN / CR. NOTE"}
                        </span>
...
<span className="text-sm font-black text-brand-secondary">Ledger</span>
                  ...
                  {/* 3-Column Meta & Customer Block */}
                  <div className="flex border border-gray-300 rounded mb-4 text-[10px] text-gray-700">
                    
                    {/* BILL TO */}
                    <div className="flex-1 p-3 border-r border-gray-300">
                      <p className="text-[9px] font-bold text-gray-400 uppercase mb-2">Bill To</p>
                      <p className="font-bold text-sm text-gray-900 mb-1">{invoice.customerName}</p>
                      <p className="font-medium text-gray-600 mb-0.5">Address: {invoice.shippingAddress || "test"}</p>
                      {invoice.customerPhone && <p className="font-medium text-gray-600 mb-0.5">Phone Number: {invoice.customerPhone}</p>}
                      <p className="font-medium text-gray-600">Place of Supply: {invoice.isInterstate ? "Inter-state" : "Intra-state"}</p>
                    </div>

                    {/* SHIP TO */}
                    <div className="flex-1 p-3 border-r border-gray-300">
                      <p className="text-[9px] font-bold text-gray-400 uppercase mb-2">Ship To</p>
                      <p className="font-bold text-sm text-gray-900 mb-1">{invoice.customerName}</p>
                      <p className="font-medium text-gray-600 mb-0.5">Address: {invoice.shippingAddress || "best"}</p>
                      {invoice.customerPhone && <p className="font-medium text-gray-600 mb-0.5">Phone Number: {invoice.customerPhone}</p>}
                      <p className="font-medium text-gray-600">Place of Supply: {invoice.isInterstate ? "Inter-state" : "Intra-state"}</p>
                    </div>

                    {/* META INFO */}
                    <div className="w-[30%] p-3 flex flex-col gap-3 font-medium bg-gray-50/30">
                      <div className="grid grid-cols-2 gap-2">
                        <div>
                          <p className="text-[9px] text-gray-400 font-bold mb-0.5">Sales Return No:</p>
                          <p className="text-gray-900 font-bold">{invoice.salesReturnNumber || "-"}</p>
                        </div>
                        <div>
                          <p className="text-[9px] text-gray-400 font-bold mb-0.5">Sales Return Date:</p>
                          <p className="text-gray-900 font-bold">{formattedDate}</p>
                        </div>
                      </div>
                      <div className="grid grid-cols-2 gap-2">
                        <div>
                          <p className="text-[9px] text-gray-400 font-bold mb-0.5">Payment Terms:</p>
                          <p className="text-gray-900 font-bold">0 Days</p>
                        </div>
                        <div>
                          <p className="text-[9px] text-gray-400 font-bold mb-0.5">Due Date:</p>
                          <p className="text-gray-900 font-bold">{invoice.dueDate ? new Date(invoice.dueDate).toLocaleDateString() : "-"}</p>
                        </div>
                      </div>
                      <div>
                        <p className="text-[9px] text-gray-400 font-bold mb-0.5">LIC number:</p>
                        <p className="text-gray-900 font-bold">4321</p>
                      </div>
                    </div>
                  </div>
...
<span className="text-sm font-black text-brand-secondary">Ledger</span>
                ...
                <div className="grid grid-cols-2 border-y border-gray-300 bg-gray-50/60 px-4 py-2 mb-4 text-[10px] font-bold text-gray-700">
                   <p>Sales Return No.: <span className="font-mono text-gray-950 font-extrabold">{invoice.salesReturnNumber || "1"}</span></p>
                   <p className="text-right">Sales Return Date: <span className="font-mono text-gray-950 font-extrabold">{formattedDate}</span></p>
                </div>
...
<span className="font-black text-brand-secondary">Ledger</span>
          ...
          >
             <div className="text-center space-y-0.5">
               <p className="font-bold text-xs uppercase tracking-wider">SALES RETURN / CR. NOTE</p>
             ...
             <div className="border-t border-dashed border-gray-300 pt-2 space-y-0.5">
               <p className="flex justify-between"><span>Invoice Number:</span><span className="font-bold">#{invoice.salesReturnNumber || "1"}</span></p>
               <p className="flex justify-between"><span>Sales Return Date:</span><span>{formattedDate}</span></p>
