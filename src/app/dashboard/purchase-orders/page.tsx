...
export default function CreditNotePage() {
  ...
  const [purchaseOrders, setCreditNotes] = useState<CreditNote[]>([]);
  ...
  const fetchCreditNotes = async (userId: string) => {
    try {
      setLoading(true);
      const q = query(collection(db, "purchaseOrders"), where("userId", "==", userId));
  ...
  const handleDelete = async (id: string, noteNumber: string) => {
    if (!confirm(`Delete Purchase Order ${noteNumber}? This cannot be undone.`)) return;
    try {
  ...
  const now = new Date();
  const filtered = purchaseOrders.filter((c) => {
    if (search && !c.customerName.toLowerCase().includes(search.toLowerCase()) && !c.purchaseOrderNumber.toLowerCase().includes(search.toLowerCase())) return false;
      ...
      <div className="bg-white border border-gray-200 rounded-xl shadow-xs overflow-hidden flex-1 flex flex-col">
        ...
        <div className="p-4 flex flex-col sm:flex-row gap-3 items-center border-b border-gray-100 bg-gray-50/40">
          <div className="relative w-full sm:w-64">
            ...
            <input
              type="text"
              placeholder="Search party or purchase order no..."
              value={search}
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-gray-50 border-b border-gray-100 text-[11px] uppercase tracking-wider text-gray-500">
                ...
                <th className="p-4 font-bold">Purchase Order Number</th>
              ...
              {loading ? (
                ...
                <tr>
                  <td colSpan={7} className="p-16 text-center">
                    <div className="flex flex-col items-center justify-center text-gray-400">
                      ...
                      <Link href="/dashboard/purchase-orders/create" className="mt-4 text-sm font-bold text-indigo-600 hover:underline">
                        + Create Purchase Order
                      </Link>
                    </div>
                  </td>
                ...
                filtered.map((cn) => (
                  <tr
                    ...
                    onClick={() => router.push(`/dashboard/purchase-orders/${cn.id}`)}
                  >
                    ...
                    <td className="p-4 font-bold text-gray-800 uppercase tracking-wide">{cn.purchaseOrderNumber}</td>
                    ...
                    <td className="p-4 text-center" onClick={(e) => e.stopPropagation()}>
                      <div className="flex items-center justify-center gap-2 opacity-100 transition">
                        ...
                        <button
                          onClick={() => router.push(`/dashboard/purchase-orders/edit/${cn.id}`)}
                        ...
                        <button
                          onClick={() => handleDelete(cn.id, cn.purchaseOrderNumber)}
