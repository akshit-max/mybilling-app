...
export default function ViewCreditNote() {
  ...
  if (loading) {
    return (
      <div className="min-h-[70vh] flex items-center justify-center text-gray-400 gap-2 font-sans">
        ...
        <span className="text-xs font-semibold uppercase tracking-wider">Loading Purchase Order details...</span>
      </div>
    );
  }
  ...
  if (!purchaseOrder) {
    return (
      <div className="min-h-[70vh] flex flex-col items-center justify-center text-gray-400 gap-3 font-sans">
        ...
        <span className="text-sm font-semibold uppercase tracking-wider">Purchase Order details not found</span>
      </div>
    );
  }
  ...
  const formattedDate = purchaseOrder.date
    ...
    ? typeof (purchaseOrder.createdAt as any).toDate === "function"
      ? (purchaseOrder.createdAt as any).toDate().toLocaleDateString()
      : new Date(purchaseOrder.createdAt as any).toLocaleDateString()
    : new Date().toLocaleDateString();

  const totalQty = purchaseOrder.items
    ? purchaseOrder.items.reduce((acc, item) => acc + (Number(item.qty) || 0), 0)
    : 0;

  const totalTaxAmount = purchaseOrder.items
    ? purchaseOrder.items.reduce((acc, item) => {
        const taxRate = item.tax || (purchaseOrder.gstEnabled ? 18 : 0);
  ...
  return (
    <div className="flex flex-col flex-1 min-w-0 font-sans bg-gray-50/60 min-h-screen">
      ...
      <div className="screen-only-container flex flex-col flex-1">
        ...
        <div className="bg-white border-b border-gray-200 px-6 py-3 flex items-center justify-between shrink-0 shadow-xs">
          <div className="flex items-center gap-2">
            <Link href="/dashboard/purchase-orders" className="text-gray-400 hover:text-gray-700 transition">
              <ArrowLeft size={16} />
            </Link>
            ...
            <div className="relative">
              ...
              {isMoreOpen && (
                <div className="absolute right-0 mt-1 bg-white border border-gray-200 rounded-lg shadow-md py-1 w-32 z-50 text-xs font-semibold">
                  <Link href={`/dashboard/purchase-orders/edit/${id}`} className="block px-4 py-2 text-gray-700 hover:bg-gray-50">Edit Purchase Order</Link>
          ...
          >
             <div>
                ...
                <div className="mb-4 space-y-0.5 text-[10px]">
                   ...
                   <p className="text-xs font-extrabold text-gray-900">{purchaseOrder.customerName}</p>
                   {showPhone && purchaseOrder.customerPhone && <p className="text-gray-650 font-semibold">Mobile: {purchaseOrder.customerPhone}</p>}
                   {purchaseOrder.customerGSTIN && <p className="text-gray-650 font-mono">GSTIN: {purchaseOrder.customerGSTIN}</p>}
                </div>
                ...
                <div className="border border-gray-300 rounded overflow-hidden mb-4">
                   <table className="w-full text-[10px] text-left border-collapse">
                      ...
                      <tbody className="divide-y divide-gray-250 text-gray-700 font-semibold">
                         {purchaseOrder.items && purchaseOrder.items.map((item, idx) => {
                           const taxRate = item.tax || (purchaseOrder.gstEnabled ? 18 : 0);
                           return (
                ...
                <div className="flex justify-between items-start text-[10px] pt-2">
                   ...
                   <div className="w-64 space-y-1 font-mono text-right text-gray-500 font-bold border-t border-dashed border-gray-350 pt-2">
                      ...
                      {purchaseOrder.gstEnabled && (
                        purchaseOrder.isInterstate ? (
                          <div className="flex justify-between text-gray-500">
                            <span>IGST</span>
                            <span>₹{(purchaseOrder.igst || 0).toFixed(2)}</span>
                          </div>
                        ) : (
                          <>
                            <div className="flex justify-between text-gray-500">
                              <span>CGST</span>
                              <span>₹{purchaseOrder.cgst.toFixed(2)}</span>
                            </div>
                            <div className="flex justify-between text-gray-500">
                              <span>SGST</span>
                              <span>₹{purchaseOrder.sgst.toFixed(2)}</span>
                            </div>
                          </>
                        )
                      )}
                      ...
                      {purchaseOrder.discountAmount > 0 && (
                        <div className="flex justify-between text-brand-tertiary">
                           <span>Discount</span>
                           <span>-₹{purchaseOrder.discountAmount.toFixed(2)}</span>
                        </div>
                      )}
             ...
             <div className="mt-8 pt-2 flex flex-col justify-between">
                <div className="flex justify-between items-end text-[9px] text-gray-400">
                  <div>
                    ...
                    <p className="italic font-bold text-gray-800 mt-0.5">{numberToWords(purchaseOrder.total)} Rupees Only</p>
                  </div>
                  {(purchaseOrder.signatureType === "empty" || purchaseOrder.signatureType === "upload") && (
                    <div className="text-right space-y-1 w-44">
                      ...
                      {purchaseOrder.signatureType === "empty" ? (
                        ...
                        <div className="h-12 border border-gray-200 rounded flex items-center justify-center p-1 bg-white overflow-hidden">
                          <img src={purchaseOrder.signatureImage} alt="Signature" className="max-h-full max-w-full object-contain" />
                        </div>
                      )}
                    </div>
                  )}
      ...
      {showSMSModal && (
        <SMSModal
          customerName={purchaseOrder?.customerName || "Customer"}
          invoiceNumber={(purchaseOrder as any)?.invoiceNumber || (purchaseOrder as any)?.quotationNumber || (purchaseOrder as any)?.challanNumber || (purchaseOrder as any)?.proformaInvoiceNumber || (purchaseOrder as any)?.orderNumber || (purchaseOrder as any)?.returnNumber || (purchaseOrder as any)?.debitNoteNumber || (purchaseOrder as any)?.creditNoteNumber || (purchaseOrder as any)?.receiptNumber || (purchaseOrder as any)?.id || "N/A"}
