...
import { collection, getDocs, query, where, doc, getDoc, updateDoc, addDoc, orderBy } from "firebase/firestore";
...
export default function StaffDetailView() {
  ...
  const handleMakePayment = async () => {
    ...
    try {
      ...
      // Update staff balance (assuming positive balance means business owes staff money)
  ...
  // Payments in current month view
  ...
  const totalPayments = currentMonthTransactions.reduce((acc, t) => acc + t.amount, 0);
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex font-sans">
      ...
      {/* Main Content Area */}
      <main className="flex-1 max-w-[1200px] flex flex-col h-[calc(100vh-60px)] overflow-y-auto bg-gray-50/50">
        ...
        <header className="bg-white border-b border-gray-200 px-6 py-4 flex items-center justify-between sticky top-0 z-20 shadow-xs">
          ...
          <div className="flex items-center gap-3">
            <button className="flex items-center gap-2 border border-gray-200 px-3 py-2 rounded text-xs font-semibold text-gray-700 hover:bg-gray-50 transition">
              <Download size={14} />
              <span>Download Salary Slip</span>
            </button>
            <button 
              onClick={() => setShowPaymentModal(true)}
              className="flex items-center gap-1 bg-indigo-600 hover:bg-indigo-700 text-white px-5 py-2 rounded text-xs font-bold transition shadow-sm"
            >
              <CreditCard size={14} />
                    ...
                    {transactions.length === 0 ? (
                      ...
                      transactions.map(t => (
                        <tr key={t.id} className="hover:bg-gray-50/50 transition">
                          ...
                          <td className="px-6 py-3.5">
                            <span className="text-xs font-bold text-green-600">↑ ₹{t.amount.toLocaleString("en-IN", { minimumFractionDigits: 2 })}</span>
                          </td>
...
fs.writeFileSync("d:\\Billing-app\\billing-app\\src\\app\\dashboard\\staff\\[id]\\page.tsx", fileContent, "utf-8");
