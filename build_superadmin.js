...
import { 
  ...
  DollarSign, BarChart3, AlertCircle, RefreshCw, XCircle, SearchX
} from "lucide-react";
  ...
  const init = async () => {
      ...
      try {
        ...
        usersSnap.forEach(doc => {
          ...
          // Estimate MRR / ARR Based on Plan
          let mrr = 0;
          let arr = 0;
          ...
          const updatedDate = parseDate(d.updatedAt || d.createdAt);
          ...
          // If paid and updated after creation by > 1 day, proxy as upgrade
          if (d.isPaid && (updatedDate.getTime() - createdDate.getTime()) > 86400000) {
            eventLog.push({
              id: \`upg_\${uid}\`,
        ...
        eventLog.sort((a, b) => b.date.getTime() - a.date.getTime());

        setCompanies(companyList);
        // Only keep last 50 activities for performance
    ...
    <div className="flex-1 flex flex-col bg-[#F8FAFC] overflow-y-auto font-sans h-full text-slate-800">
      ...
      <main className="w-full max-w-[1600px] mx-auto p-6 md:p-8 space-y-8">
        ...
        {!selectedCompany ? (
                              ...
                              <tr key={c.uid} className="hover:bg-slate-50/50 transition-colors group">
                                ...
                                <td className="py-4 px-6">
                                  <div className="flex flex-col gap-1.5">
                                    ...
                                    {c.isPaid && <div className="text-xs text-slate-400 font-medium flex items-center gap-1"><RefreshCw size={10}/> Renews {c.renewalDate?.toLocaleDateString()}</div>}
                                  </div>
                                </td>
                                <td className="py-4 px-6">
                                  {c.isPaid ? (
                                ...
                                <td className="py-4 px-6">
                                  <div className="text-xs text-slate-500 font-medium">Joined: {c.createdAt.toLocaleDateString("en-IN", { month: 'short', year: 'numeric'})}</div>
                                  <div className="text-xs text-slate-400 mt-0.5 flex items-center gap-1"><Clock size={10}/> Active: {c.updatedAt.toLocaleDateString("en-IN")}</div>
                                </td>
                            ...
                            <div key={\`\${act.id}_\${i}\`} className="flex gap-4 relative">
                              ...
                              <div className="pt-1.5 pb-2">
                                ...
                                <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest mt-1.5">
                                  {act.date.toLocaleDateString("en-IN", { month: 'short', day: 'numeric'})} • {act.date.toLocaleTimeString("en-IN", { hour: '2-digit', minute:'2-digit' })}
                                </p>
                              </div>
                            </div>
                          ))
          ...
          /* --- DETAILED VIEW --- */
          <div className="animate-in slide-in-from-right-8 duration-300 space-y-4 max-w-5xl mx-auto">
             <button 
                  ...
                  <div className="space-y-4">
                     ...
                     <div className="bg-slate-50 p-6 rounded-2xl border border-slate-200 grid grid-cols-2 gap-6 relative overflow-hidden">
                        ...
                        <div className="pt-4 border-t border-slate-200">
                           ...
                           <p className="text-sm font-bold text-slate-700 mt-1">{selectedCompany.renewalDate ? selectedCompany.renewalDate.toLocaleDateString('en-IN') : "N/A"}</p>
                        </div>
                     </div>
                  </div>
                     ...
                     <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm h-full">
                        ...
                        {selectedCompany.employeeCount > 0 ? (
                          ...
                          <div className="border-t border-slate-100 pt-5 text-center text-xs text-slate-400 font-medium">
                            No additional staff members added.
                          </div>
                        )}
                     </div>
                  </div>
...
console.log('Superadmin dashboard upgraded!');
